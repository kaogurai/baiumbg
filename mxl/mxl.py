...
class MXL(commands.Cog):
    ...
    def __init__(self):
        ...
        self.forum_logout_endpoint = (
            "https://forum.median-xl.com/ucp.php?mode=logout&sid={}"
        )
        ...
        self.item_css = (
            data_manager.bundled_data_path(self) / "item_style.css"
        ).as_posix()
        self.flickr_client = None
        self.thread_pool = concurrent.futures.ThreadPoolExecutor(
            max_workers=3, thread_name_prefix="mxl_"
        )
    ...
    async def auctions_search(self, ctx, *, title: str):
        ...
        matching_auctions = [
            embed for embed in embeds if re.search(title, embed.title, re.IGNORECASE)
        ]
        if not matching_auctions:
            await ctx.send(
                "There are no active auctions that match that description at the moment."
            )
            return
    ...
    async def generate_crafted_images(self, ctx, enabled: bool):
        ...
        **Warning**: Do not use with large amounts of crafted items - the command will most likely fail and will make the bot reconnect to Discord.
        ...
        await ctx.send(
            f'generate_crafted_images {"enabled" if enabled else "disabled"}.'
        )
    ...
    async def post_template(self, ctx, pastebin: str = None):
        ...
        if pastebin == "reset":
            await self._config.member(ctx.author).post_template.set(
                DEFAULT_TRADE_POST_TEMPLATE
            )
        ...
        async with aiohttp.request(
            "GET", self.pastebin_raw_endpoint.format(pastebin_key)
        ) as response:
            ...
            if response.status != 200:
                await ctx.send(
                    f"Couldn't fetch template from Pastebin! Maybe pastebin.com is down?"
                )
                return
    ...
    async def pricecheck(self, ctx, *, item: str):
        ...
        if not config["forum_username"]:
            await ctx.send(
                f"No forum account is currently configured for this server. Use `{ctx.prefix}mxl config` to set one up."
            )
            return
        ...
        async with aiohttp.request(
            "POST",
            self.tradecenter_enpoint,
            data={"search": item, "submit": ""},
            cookies=config["forum_cookies"],
        ) as pricecheck_response:
        ...
        if dom.find(not_logged_in_function):
            ...
            if error == LoginError.INCORRECT_USERNAME:
                await ctx.send(
                    f"Incorrect forum username. Please set a valid one using `{ctx.prefix}mxl config username`."
                )
                return
            elif error == LoginError.INCORRECT_PASSWORD:
                await ctx.send(
                    f"Incorrect forum password. Please set the proper one using `{ctx.prefix}mxl config password`."
                )
                return
            elif error == LoginError.LOGIN_ATTEMPTS_EXCEEDED:
                await ctx.send(
                    f"Maximum login attempts exceeded. Please login to the forum manually (with the configured account) and solve the CAPTCHA."
                )
                return
            ...
            if dom.find(not_logged_in_function):
                await ctx.send(
                    "Couldn't login to the forums. Please report this to the plugin author."
                )
                return
    ...
    async def logout_forum(self, ctx):
        ...
        async with aiohttp.request(
            "GET",
            self.forum_logout_endpoint.format(config["forum_cookies"]["MedianXL_sid"]),
            cookies=config["forum_cookies"],
        ) as logout_response:
    ...
    async def logout_armory(self, ctx):
        ...
        async with aiohttp.request(
            "GET", self.armory_logout_endpoint, cookies=config["armory_cookies"]
        ) as logout_response:
            dom = BeautifulSoup(await logout_response.text(), "html.parser")
    ...
    async def armory_dump(self, ctx, *characters):
        ...
        if not config["pastebin_api_key"]:
            await ctx.send(
                f"Pastebin API key hasn't been configured yet. Configure one using `{ctx.prefix}mxl config pastebin_api_key`."
            )
            return

        if not config["armory_username"]:
            await ctx.send(
                f"Armory account hasn't been configured yet. Configure one using `{ctx.prefix}mxl config armory_username/armory_password`."
            )
            return

        if (
            not config["flickr_api_key"]
            or not config["flickr_api_secret"]
            and self.flickr_client is None
        ):
            await ctx.send(
                f"Flickr API key/secret hasn't been configured yet. Configure using `{ctx.prefix}mxl config flickr_api_key/flickr_api_secret`."
            )
            return

        if not self.flickr_client:
            self.flickr_client = flickrapi.FlickrAPI(
                config["flickr_api_key"], config["flickr_api_secret"], format="xmlnode"
            )

        if not self.flickr_client.token_valid(perms="write"):
            await ctx.send(
                f"Missing flickr client token. Use `{ctx.prefix}mxl flickr` to configure one."
            )
            return
        ...
        for character in characters:
            async with aiohttp.request(
                "GET",
                self.armory_character_endpoint.format(character),
                cookies=config["armory_cookies"],
            ) as character_response:
            ...
            if dom.find(action="login.php"):
                ...
                if error:
                    await ctx.send(
                        "Incorrect armory username/password or armory is not reachable."
                    )
                    return
                async with aiohttp.request(
                    "GET",
            ...
            if dom.h1 and "not allowed" in dom.h1.text:
                await ctx.send(
                    f"Character {character}'s armory is private! Please log into the armory and make it publicly viewable to dump its items. Skipping."
                )
                continue
        ...
        (
            ...
            generation_error,
            unknown_arguments,
        ) = await items.to_trade_post(
            user_config["post_template"],
            self.flickr_client,
            self.item_css,
            user_config,
            config["flickr_cache"],
        ...
        if generation_error == PostGenerationErrors.IMAGE_UPLOAD_FAILED:
            await ctx.send(
                "An error occurred while uploading an item's image to flickr. Try again later."
            )
            return
        elif generation_error == PostGenerationErrors.UNKNOWN:
            await ctx.send(
                "An unknown error occurred while generating your trade post. Try again later."
            )
            return

        if unknown_arguments:
            await ctx.channel.send(
                f'{ctx.author.mention} Warning: Unknown template macros: {", ".join(unknown_arguments)}'
            )

        pastebin_link = await self._create_pastebin(
            post, f'MXL trade post for characters: {", ".join(characters)}'
        )
        channel = ctx.author.dm_channel or await ctx.author.create_dm()
        if pastebin_link:
            await ctx.channel.send(
                f"{ctx.author.mention} Dump successful. Check your DMs."
            )
            await channel.send(
                f'Armory dump for characters {", ".join(characters)}: {pastebin_link}'
            )
            return

        await ctx.send(
            "Couldn't create the trade post paste - 24h limit is probably reached. Check your DMs."
        )
    ...
    async def _forum_login(self):
        ...
        async with aiohttp.request(
            "POST",
            self.forum_login_endpoint,
            data={
                "username": config["forum_username"],
    ...
    async def _armory_login(self):
        ...
        async with aiohttp.request(
            "POST",
            self.armory_login_endpoint,
            data={"user": config["armory_username"], "pass": config["armory_password"]},
            cookies={"PHPSESSID": session_id},
        ) as login_response:
            dom = BeautifulSoup(await login_response.text(), "html.parser")

        if not dom.contents:
            config["armory_cookies"] = {"PHPSESSID": session_id}
            await self._config.set(config)
            return False, config
        ...
        for item in item_dump:
            ...
            if item_name == "Eye of Wisdom":
                class_match = re.search(
                    r"(Amazon|Assassin|Barbarian|Druid|Necromancer|Paladin|Sorceress)",
                    item.text,
                )
                if not class_match:
                    items.increment_set_item(
                        "Unknown", item_name, character, item.parent.parent
                    )
                    continue
                ...
                items.increment_set_item(
                    SETS[item_name], item_name, character, item.parent.parent
                )
                continue
            ...
            if set_match:
                ...
                items.increment_set_item(
                    set_name, item_name, character, item.parent.parent
                )
                continue
            ...
            if (
                item.span["class"][0] == "color-white"
                or item.span["class"][0] == "color-blue"
            ) and item_name not in MISC_ITEMS:
                base_name = (
                    item_name + " [eth]"
                    if "Ethereal" in item.text
                    else "".join(item_name.split("Superior "))
                )
            ...
            if shrine_match:
                ...
                items.increment_shrine(
                    shrine_name, character, item.parent.parent, amount
                )
                continue
            ...
            if item_name in SHRINE_VESSELS:
                vessel_amount = int(
                    (
                        re.search(
                            r"Quantity: ([0-9]+)", item.find(class_="color-grey").text
                        )
                    ).group(1)
                )
                shrine_name = VESSEL_TO_SHRINE[item_name]
                items.increment_shrine(
                    shrine_name, character, item.parent.parent, vessel_amount
                )
                continue
            ...
            if item_name == "Arcane Cluster":
                ...
                items.increment_other(
                    "Arcane Crystal", character, item.parent.parent, crystals_amount
                )
                continue
            ...
            if AC_shards_match:
                ...
                items.increment_other(
                    "Arcane Crystal", character, item.parent.parent, amount
                )
                continue

            if (
                item.span["class"][0] == "color-orange"
                and item_name not in MISC_ITEMS
                and item_name not in TROPHIES
            ):
                if user_config["crafted_as_base"]:
                    items.increment_shrine_base(
                        item_name, character, item.parent.parent
                    )
                    continue
            ...
            if item_name in TROPHIES:
                items.increment_trophy(
                    item_name, character, item.parent.parent, item_quantity
                )
                continue
