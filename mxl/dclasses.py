...
class ItemDump:
    ...
    def increment_set_item(self, set_name, item_name, character, html):
        self.sets.setdefault(set_name, Set(name=set_name)).items.setdefault(
            item_name, Item(name=item_name)
        ).increment(character, html)
    ...
    def increment_trophy(self, item_name, character, html, amount=1):
        self.trophies.setdefault(item_name, Item(name=item_name)).increment(
            character, html, amount
        )

    def increment_shrine(self, item_name, character, html, amount=1):
        self.shrines.setdefault(item_name, Item(name=item_name)).increment(
            character, html, amount
        )

    def increment_crafted(self, item_name, character, html):
        self.crafted.setdefault(item_name, Item(name=item_name)).increment(
            character, html
        )

    def increment_other(self, item_name, character, html, amount=1):
        self.other.setdefault(item_name, Item(name=item_name)).increment(
            character, html, amount
        )

    async def to_trade_post(
        self,
        post_template,
        flickr_client,
        css_file,
        ...
        for set_ in sorted(self.sets.values(), key=lambda k: k.name):
            ...
            for item in sorted(set_.items.values(), key=lambda k: k.name):
                sets_str += (
                    f"[item]{item.name}[/item] x{item.amount}\n"
                    if item.amount > 1
                    else f"[item]{item.name}[/item]\n"
                )
        ...
        for item in sorted(self.crafted.values(), key=lambda k: k.name):
            crafted_str += (
                f"[color=#FAAA23]{item.name}[/color] x{item.amount}\n"
                if item.amount > 1
                else f"[color=#FAAA23]{item.name}[/color]\n"
            )
            ...
            if user_config["generate_crafted_images"]:
                ...
                for tag in item.html:
                    ...
                    image_file = os.path.join(
                        tempfile.gettempdir(),
                        "".join(random.choice("0123456789ABCDEF") for i in range(12))
                        + ".png",
                    )
                    ...
                    try:
                        ...
                        image_link = (
                            flickr_client.photos.getSizes(photo_id=image_id)
                            .sizes[0]
                            .size[-1]["source"]
                        )
                    except:
                        return (
                            None,
                            cache_update,
                            PostGenerationErrors.IMAGE_UPLOAD_FAILED,
                            None,
                        )
                    finally:
        ...
        for item in sorted(self.rw_bases.values(), key=lambda k: k.name):
            rw_bases_str += (
                f"[color=#808080]{item.name}[/color] x{item.amount}\n"
                if item.amount > 1
                else f"[color=#808080]{item.name}[/color]\n"
            )

        shrine_bases_str = ""
        for item in sorted(self.shrine_bases.values(), key=lambda k: k.name):
            shrine_bases_str += (
                f"[color=#FFFF00]{item.name}[/color] x{item.amount}\n"
                if item.amount > 1
                else f"[color=#FFFF00]{item.name}[/color]\n"
            )
        ...
        for item in sorted(self.trophies.values(), key=lambda k: k.name):
            trophies_str += (
                f"[color=#FF7F50]{item.name}[/color] x{item.amount}\n"
                if item.amount > 1
                else f"[color=#FF7F50]{item.name}[/color]\n"
            )
        ...
        return (
            post_template.format_map(template_args),
            cache_update,
            None,
            template_args.missing_keys,
        )
